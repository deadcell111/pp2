# Dictionary of movies

movies = [
{
"name": "Usual Suspects", 
"imdb": 7.0,
"category": "Thriller"
},
{
"name": "Hitman",
"imdb": 6.3,
"category": "Action"
},
{
"name": "Dark Knight",
"imdb": 9.0,
"category": "Adventure"
},
{
"name": "The Help",
"imdb": 8.0,
"category": "Drama"
},
{
"name": "The Choice",
"imdb": 6.2,
"category": "Romance"
},
{
"name": "Colonia",
"imdb": 7.4,
"category": "Romance"
},
{
"name": "Love",
"imdb": 6.0,
"category": "Romance"
},
{
"name": "Bride Wars",
"imdb": 5.4,
"category": "Romance"
},
{
"name": "AlphaJet",
"imdb": 3.2,
"category": "War"
},
{
"name": "Ringing Crime",
"imdb": 4.0,
"category": "Crime"
},
{
"name": "Joking muck",
"imdb": 7.2,
"category": "Comedy"
},
{
"name": "What is the name",
"imdb": 9.2,
"category": "Suspense"
},
{
"name": "Detective",
"imdb": 7.0,
"category": "Suspense"
},
{
"name": "Exam",
"imdb": 4.2,
"category": "Thriller"
},
{
"name": "We Two",
"imdb": 7.2,
"category": "Romance"
}
]
#1
def five_five(movie):
    return movie.get("imdb",0) > 5.5
print(five_five(movies[0])) #True
print(five_five(movies[8])) #False 
#2
def movies_ff(movies):
    return [movie for movie in movies if five_five(movie)]
print(movies_ff(movies))
#3
def movies_category(movies, category):
    return [movie for movie in movies if movie.get("category") == category]
print(movies_category(movies, "Thriller"))
#4
def average_imdb(movies):
    total_score = sum(movie["imdb"] for movie in movies)
    return total_score / len(movies) if movies else 0

print(average_imdb(movies))
#5
def avg_imdb_ctg(movies, category):
    category_movies = [movie for movie in movies if movie.get("category") == category]
    return average_imdb(category_movies) if category_movies else 0

print(average_imdb(movies, "Romance"))